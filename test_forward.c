...
static void cpu_dequant_gemv(const uint8_t *w, const uint8_t *ws,
                              const float *x, float *y, int M, int K) {
    for (int r = 0; r < M; r++) {
        float sum = 0.0f;
        for (int k = 0; k < K / 2; k++) {
            uint8_t packed = w[r * (K/2) + k];
            int group = k / 8;
            float sc = f8e4m3_to_f32(ws[r * (K/16) + group]);
            float ws2 = 2.0f;
            sum += e2m1_to_f32(packed & 0xF) * ws2 * sc * x[k*2];
            sum += e2m1_to_f32((packed >> 4) & 0xF) * ws2 * sc * x[k*2+1];
        }
        y[r] = sum;
    }
}

/* GPU kernel declared in ds4_cuda_forward.cu */
...
int main(int argc, char **argv) {
    ...
    /* CPU GEMV */
    cpu_dequant_gemv(hw, hs, hx, hy_cpu, M, K);
    ...
    CHECK_CUDA(cudaMalloc((void**)&dw, (size_t)M*(K/2)));
    CHECK_CUDA(cudaMemcpy(dw, hw, (size_t)M*(K/2), cudaMemcpyHostToDevice));
    CHECK_CUDA(cudaMalloc((void**)&ds_d, (size_t)M*(K/16)));
    CHECK_CUDA(cudaMemcpy(ds_d, hs, (size_t)M*(K/16), cudaMemcpyHostToDevice));
    ...
    launch_gemv_nvfp4(dx, dw, ds_d, dy, M, K);
